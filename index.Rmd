...
`ResOpsUS` can be downloaded in full and read into R using `starfit` function `read_reservoir_data()`. Alternatively, the same function can access `ResOpsUS` data directly.

```{r}
read_reservoir_data("https://raw.githubusercontent.com/IMMM-SFA/starfit/master/inst/extdata/", 297) ->
  Libby_res_data
...
To convert these time series data into reservoir operating policies!
...
That's it. If we want to look at what this parameters imply in terms of weekly storage targets:
